...
# ── SQLite setup ──────────────────────────────────────────────────────────────
DB_PATH = os.path.join(os.path.abspath(os.path.dirname(__file__)), "outreach.db")
...
MODEL_COST = {
    ...
    "gpt-4o-mini-search-preview":{"in": 0.15/1e6, "out": 0.60/1e6},
    "gpt-4o":                    {"in": 2.50/1e6, "out":10.00/1e6},
    "gpt-4o-search-preview":     {"in": 2.50/1e6, "out":10.00/1e6},
}

def calc_cost(model, inp, out):
    c = MODEL_COST.get(model, {"in": 0.15/1e6, "out": 0.60/1e6})
    return round(c["in"] * inp + c["out"] * out, 6)

def log_tokens(run_id, endpoint, model, usage_dict):
    """Persist token usage from an OpenAI response usage object."""
    if not usage_dict:
        return
    ...
    try:
        with get_db() as conn:
            conn.execute(
                "INSERT INTO token_usage (run_id, endpoint, model, input_tokens, output_tokens, cost_usd) "
                "VALUES (?,?,?,?,?,?)",
                (run_id, endpoint, model, inp, out, cost)
            )
            conn.commit()
    except Exception:
        pass
...
# ── HTML email builder ────────────────────────────────────────────────────────
def build_html_email(body_text, sender_name, sender_role):
    """Convert plain-text email body to clean HTML, preserving line breaks and paragraphs."""
    paragraphs = body_text.strip().split("\n\n")
    html_parts = []
    for para in paragraphs:
        escaped = htmlmod.escape(para).replace("\n", "<br>")
  ...
  <table width="100%" cellpadding="0" cellspacing="0" style="padding:32px 16px">
    <tr><td align="center">
      <table width="600" cellpadding="0" cellspacing="0"
        ...
        <!-- header -->
        <tr><td style="background:linear-gradient(135deg,#2563eb,#0ea5e9);padding:20px 32px">
          <span style="color:#ffffff;font-size:18px;font-weight:700;letter-spacing:-.3px">
            Redcliffe Labs
          </span>
          <span style="color:rgba(255,255,255,.75);font-size:13px;margin-left:10px">
            {htmlmod.escape(sender_role)}
          </span>
        </td></tr>
        <!-- body -->
        <tr><td style="padding:32px;color:#1e293b;font-size:15px;line-height:1.75">
          {body_html}
        </td></tr>
        <!-- footer -->
        <tr><td style="background:#f8fafc;padding:16px 32px;border-top:1px solid #e2e8f0">
          <p style="margin:0;font-size:12px;color:#94a3b8">
            {htmlmod.escape(sender_name)} · {htmlmod.escape(sender_role)} · Redcliffe Labs<br>
...
# Sheet name: "sales"  (single tab)
# Columns: Type | Run ID | Date | Name | Email | Company | Event | Subject |
#          Mode | Status | Endpoint | Model | Input Tokens | Output Tokens |
#          Cost (USD) | Has Reply | Has Bounce
GSHEET_WEBHOOK_URL = os.environ.get("GSHEET_WEBHOOK_URL", "")

def gsheet_append(row):
    """POST one row to the Apps Script webhook. Silently skips if not configured."""
    if not GSHEET_WEBHOOK_URL:
        return
    try:
        requests.post(GSHEET_WEBHOOK_URL, json={"row": row}, timeout=10)
    except Exception:
        pass

def gsheet_update_status(recipient_email, has_reply, has_bounce):
    """Append a status-update row to the sales sheet."""
    now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    gsheet_append([
        "status", "", now, "", recipient_email, "", "", "", "",
        "replied" if has_reply else "bounced",
        "", "", 0, 0, 0, int(has_reply), int(has_bounce),
    ])
...
def openai_proxy():
    ...
    openai_key = (payload.pop("openai_key", None) or "").strip() or OPENAI_API_KEY
    if not openai_key:
...
def search_events():
    ...
    search_prompt = (
        ...
        f"SEARCH STRATEGY — run ALL of these searches:\n\n"
        f"Search 1 — Event aggregators (comprehensive databases):\n"
        f"  - 10times.com: search '{category} India {this_month}' and '{category} India {next_month}'\n"
        f"  - eventzilla.in, townscript.com, eventshigh.com, allevents.in: same queries\n\n"
        f"Search 2 — Broad Google with event format variants:\n"
        f"  - '{category} summit India {this_month}'\n"
        f"  - '{category} conclave India 2026'\n"
        f"  - '{category} conference roundtable forum India {date_range}'\n"
        f"  - '{category} India {date_range} agenda speakers'\n\n"
        f"Search 3 — LinkedIn (events are often ONLY posted here):\n"
        f"  - site:linkedin.com/events '{category}' India 2026\n"
        f"  - site:linkedin.com/posts '{category}' summit India {this_month}\n"
        f"  - site:linkedin.com '{category}' conclave India {date_range}\n\n"
        f"Search 4 — Industry news and media:\n"
        f"  - economictimes.com, businessline.com, hrworld.in, peoplemattersglobal.com, ethrworld.com, businesstoday.in\n"
        f"  - Search: '{category} event India {date_range}'\n\n"
        f"Search 5 — Direct event website search:\n"
        f"  - '{category} India 2026 registration' — to find official event sites\n\n"
...
def send_bulk():
    """Generate personalised emails for every speaker and send via Gmail SMTP."""
    data            = request.get_json(force=True)
    openai_key      = resolve_openai_key(data)
    if not openai_key:
        return jsonify({"error": "OpenAI API key required. Enter it in the top-right field or add OPENAI_API_KEY to .env"}), 400
    ...
    for person in people_to_send:
        ...
        if pid in previews_in and previews_in[pid].get("body"):
            ...
            try:
                ...
                usage   = ec.get("usage", {})
                sum_inp += usage.get("prompt_tokens", 0)
                sum_out += usage.get("completion_tokens", 0)
            except Exception as exc:
        ...
        # ── Plain text body only (no HTML box) ──────────────────────
            ...
            try:
                with get_db() as conn:
                    conn.execute(
                        ...
                        "(run_id,recipient_name,recipient_email,company,event_name,"
                        " subject,body_html,test_mode,status) VALUES (?,?,?,?,?,?,?,?,?)",
                        (run_id, name, actual_to, company, event_name,
                         msg["Subject"], body, int(test_mode), "sent"),
                    )
                    conn.commit()
            except Exception:
                pass

            # ── Sync to Google Sheets (sales tab) ───────────────────
            gsheet_append([
                "email", run_id, sent_at, name, actual_to, company,
                event_name, msg["Subject"],
                "TEST" if test_mode else "REAL", "sent",
                "send-bulk", "", 0, 0, 0, 0, 0,
            ])
    ...
    # ── Log token usage for this run ──────────────────────────────────
    gsheet_append([
        "tokens", run_id,
        datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        "", "", "", "", "", "", "",
        "send-bulk/email-generation", "gpt-4o-mini",
        sum_inp, sum_out,
        round(calc_cost("gpt-4o-mini", sum_inp, sum_out), 5),
        0, 0,
    ])
    try:
        with get_db() as conn:
            conn.execute(
                "INSERT INTO token_usage (run_id,endpoint,model,input_tokens,output_tokens,cost_usd) "
                "VALUES (?,?,?,?,?,?)",
                (run_id, "send-bulk", "gpt-4o-mini",
                 sum_inp, sum_out,
                 calc_cost("gpt-4o-mini", sum_inp, sum_out)),
            )
            conn.commit()
    except Exception:
        pass
    ...
    return jsonify({
        ...
        "token_summary": {
            ...
            "input":    sum_inp,
            "output":   sum_out,
            "cost_usd": round(calc_cost("gpt-4o-mini", sum_inp, sum_out), 5),
        },
    })
...
def find_emails():
    ...
    for person in people:
        ...
        try:
            ...
            # If person was found but email still null, try email_addresses array
...
def check_replies():
    """Connect to Gmail IMAP and check for replies and bounces for sent emails."""
    data     = request.get_json(force=True)
    email_addr   = data.get("sender_email", "").strip()
    app_password = data.get("sender_password", "").strip()
    if not email_addr or not app_password:
        return jsonify({"error": "Sender email and app password required"}), 400
    ...
    try:
        ...
        # ── Check Bounces (from MAILER-DAEMON / postmaster) ──────────
        mail.select("INBOX")
        for sender in ["MAILER-DAEMON", "postmaster"]:
            _, ids = mail.search(None, "FROM", f'"{sender}"')
            for num in (ids[0].split() if ids[0] else []):
                try:
                    _, raw = mail.fetch(num, "(RFC822)")
                    msg = emaillib.message_from_bytes(raw[0][1])
                    intended = msg.get("X-Failed-Recipients", "") or msg.get("Final-Recipient", "")
                    if intended:
                        bounce_emails.add(intended.lower().split()[-1].strip("<>"))
                except Exception:
                    pass

        # ── Check Replies (Re: subjects) ──────────────────────────────
        _, ids = mail.search(None, 'SUBJECT', '"Re:"')
        for num in (ids[0].split()[-50:] if ids[0] else []):   # last 50
            try:
                _, raw = mail.fetch(num, "(RFC822)")
                msg = emaillib.message_from_bytes(raw[0][1])
                subj = msg.get("Subject", "")
                if subj.startswith("Re:"):
                    reply_subjects.add(subj[3:].strip().lower())
            except Exception:
                pass

        mail.close(); mail.logout()
    except imaplib.IMAP4.error as exc:
        return jsonify({"error": f"IMAP login failed: {exc}"}), 401
    except Exception as exc:
        return jsonify({"error": str(exc)}), 500

    # ── Update DB + Sheets with new statuses ─────────────────────────
    try:
        with get_db() as conn:
            rows = conn.execute("SELECT id, recipient_email, subject FROM sent_emails").fetchall()
            for row in rows:
                email_lc   = (row["recipient_email"] or "").lower()
                subject_lc = (row["subject"] or "").lower().replace("[test → ", "").split("]")[-1].strip()
                has_reply  = int(subject_lc in reply_subjects)
                has_bounce = int(email_lc in bounce_emails)
                if has_reply or has_bounce:
                    new_status = "replied" if has_reply else "bounced"
                    conn.execute("UPDATE sent_emails SET status=? WHERE id=?", (new_status, row["id"]))
                    conn.execute(
                        "INSERT INTO email_checks (sent_email_id, has_reply, has_bounce) VALUES (?,?,?)",
                        (row["id"], has_reply, has_bounce),
                    )
                    gsheet_update_status(email_lc, has_reply, has_bounce)
                    checked.append({"email": email_lc, "reply": bool(has_reply), "bounce": bool(has_bounce)})
            conn.commit()
    except Exception:
        pass

    return jsonify({
        "replies_found": len(reply_subjects),
        "bounces_found": len(bounce_emails),
        "updated":       checked,
    })


@app.route("/api/token-report", methods=["GET"])
def token_report():
    """Return total token usage + cost grouped by run and model."""
    try:
        with get_db() as conn:
            rows = conn.execute("""
                SELECT run_id, endpoint, model,
                       SUM(input_tokens)  AS total_in,
                       SUM(output_tokens) AS total_out,
                       SUM(cost_usd)      AS total_cost,
                       MIN(created_at)    AS started_at
                FROM token_usage
                GROUP BY run_id, endpoint, model
                ORDER BY started_at DESC
            """).fetchall()
            grand = conn.execute(
                "SELECT SUM(input_tokens) AS ti, SUM(output_tokens) AS to_, SUM(cost_usd) AS tc FROM token_usage"
            ).fetchone()
        return jsonify({
            "rows": [dict(r) for r in rows],
            "grand_total": {
                "input_tokens":  grand["ti"]  or 0,
                "output_tokens": grand["to_"] or 0,
                "cost_usd":      round(grand["tc"] or 0, 5),
            },
        })
    except Exception as exc:
        return jsonify({"error": str(exc)}), 500
...
def email_history():
    """Return all sent emails with their current status."""
    try:
        with get_db() as conn:
            rows = conn.execute(
                "SELECT id, run_id, recipient_name, recipient_email, company, "
                "       event_name, subject, sent_at, test_mode, status "
                "FROM sent_emails ORDER BY sent_at DESC LIMIT 200"
            ).fetchall()
        return jsonify({"emails": [dict(r) for r in rows]})
    except Exception as exc:
        return jsonify({"error": str(exc)}), 500
